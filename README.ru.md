...
**Чтобы получить пользу, не нужно сначала чему-то учиться.** Установил и запустил
`git-private2public` внутри любого Git-репозитория: сразу проверяются отслеживаемые
файлы и история по встроенным правилам. Для защиты не нужны ни `init`, ни конфиг.
...
git-private2public              # сканирует и предложит включить pre-push guard
```

Это уже работает без настройки. `git-private2public scan` явно запускает ту же проверку.

Настройка нужна только для публикации: программе надо один раз сказать, куда класть публичную копию.
...
Если нужен только `guard` без автопубликации, достаточно согласиться с
zero-config предложением или выполнить `guard enable`. Guard и
автопубликация могут работать вместе, а существующий пользовательский
pre-push hook сохраняется и вызывается автоматически.
